...
## Key next steps
A key issue that KEIFCA needs to determine (and the aim for a meeting scheduled for the 9 March 2021) is the scope of the work to be undertaken in partnership with the MMO and NE with respect developing management for Goodwin Sands MCZ. The roles of the MMO and KEIFCA needs to be determined, given Fisheries Assessment (Assessment) and Fisheries Management (Management) is to be prepared for the whole site rather than two separate components for the inshore and offshore, despite the MCZ spanning across the boundary into both jurisdictions.

Development of the Assessment and Management for Margate and Long Sands SAC, which similarly to Goodwin Sands MCZ spans across the 6 NM boundary, was treated as a single entity. KEIFCA proposes that a similar approach is taken to developing management options for the Goodwin Sands MCZ here.
...
The Goodwin Sands MCZ falls partly within the jurisdiction of the MMO and partly KEIFCA, presenting challenges to develop management for the site. Ideally (conservation best practice), the entire Goodwin Sands MCZ is to be treated as a single entity for which a Fisheries Assessment and management is to be developed in a holistic manner. The key issue that needs resolving is how roles and workloads will be shared among the MMO and KEIFCA.
...
kable(conservation_targets, caption = "Table 2. Conservation targets for designated features (and one additional feature) used in developing management for the Goodwin Sands MCZ.")
...
kable(data_sets, caption = "Table 3. Data sets for designated features used in developing management for the Goodwin Sands MCZ")
```
...
kable(add_data_sets, caption = "Table 4. Additional data sets for non-designated features used in developing management for the Goodwin Sands MCZ")
```
...
We developed a script (computer code) in R statistical software which uses functions provided in an R package (PrioritizeR) to interface with integer linear programming (Gurobi). The functions provided allow specifying the conservation problem, in terms of the planning domain, planning units, conservation features data, conservation targets and constraints, such as socio-economic costs.



For the purposes of developing a  no -bottom towed gear byelaw which aims to afford protection to the design
